...
import content from "@/content/marketing.zh-CN.json";
    ...
    <div className="py-14">
      <Container>
        ...
        <div className="mt-14 rounded-2xl border border-black/10 p-8 dark:border-white/10">
          <div className="text-sm font-medium text-foreground/70">热门 AI API 模型</div>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {["全部", "LLM", "文生图", "文生视频", "图生视频", "音频"].map((t) => (
              <button
                key={t}
                type="button"
        ...
        <div className="mt-14 rounded-2xl border border-black/10 p-8 dark:border-white/10">
          ...
          <pre className="mt-6 overflow-x-auto rounded-xl bg-black/5 p-4 text-xs text-foreground/80 dark:bg-white/10">
            {`from openai import OpenAI\n\nclient = OpenAI(base_url=\"https://api.example.com/v1\", api_key=\"sk-...\")\nresponse = client.chat.completions.create(model=\"gpt-5.1\", messages=[{\"role\":\"user\",\"content\":\"你好\"}])`}
          </pre>
        </div>
