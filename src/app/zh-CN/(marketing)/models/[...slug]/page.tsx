...
export default async function ModelRoutePageZh({
  params,
}: {
  ...
  if (!variantSlug) {
    ...
    return (
      <Container className="py-12">
        <ModelsShell basePath="/zh-CN" mode="detail">
          {showBackLink ? (
          ...
          <div className="mt-4 rounded-2xl border border-black/10 p-6 dark:border-white/10">
            <div className="text-sm text-foreground/70">提供方：{series.provider}</div>
          ...
          <div className="mt-8">
            <div className="text-sm font-medium">可用版本</div>
            {series.variants.length === 0 ? (
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                {series.variants.map((v) => (
                  <Link
                    key={v.slug}
                    href={`/zh-CN/models/${series.slug}/${v.slug}`}
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <CopyButton value={v.modelId} label="复制 Model ID" />
                    </div>
  ...
  return (
    <Container className="py-12">
      <ModelVariantPlayground locale="zh-CN" series={series} variant={variant} />
    </Container>
  );
}
