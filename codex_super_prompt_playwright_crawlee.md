...
## 强约束（必须严格遵守）
- **不得**提供任何绕过登录/付费/验证码/风控的实现。  
- **不得**抓取 `*.pdf` 等若站点禁止；遵守 robots.txt（默认遵守，可加开关）。  
- 交互探索必须“安全白名单”：
  - 允许：`a[href]`（同域），`button`/`[role=button]` 且文本/aria-label 命中“展开/更多/下一页/prev/next/tab/切换/加载”等
  - 禁止：包含“购买/支付/提交/删除/注销/登录/注册/上传/发布”等关键词的按钮；禁止对 `type=submit` 表单提交；禁止触发外域跳转
- 必须速率限制：并发 ≤ 2，页面间延迟 500–1500ms 随机抖动
